...
with st.sidebar:
    ...
    inflation = st.slider("Inflation Rate", 0.0, 0.08, 0.025, 0.005, format="%.1f%%")
    ivafe     = st.slider("IVAFE Wealth Tax", 0.000, 0.005, 0.002, 0.001, format="%.1f%%")
    ...
    eq_return   = st.slider("Equity Gross Return",    0.01, 0.15, 0.070, 0.005, format="%.1f%%")
    eq_vol      = st.slider("Equity Volatility (σ)",  0.05, 0.40, 0.160, 0.005, format="%.1f%%")
    eq_income   = st.slider("Equity Income (Dividend Yield)", 0.00, 0.05, 0.015, 0.005, format="%.1f%%")
    btp_return  = st.slider("BTP Gross Return",       0.01, 0.10, 0.035, 0.005, format="%.1f%%")
    cash_return = st.slider("Cash Return",            0.00, 0.08, 0.025, 0.005, format="%.1f%%")
...
r1.metric("30-Year Survival Rate", f"{survival_rate:.1%}",
